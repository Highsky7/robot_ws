...
# [최종 수정] 불필요한 TransportHints 관련 import는 모두 삭제합니다.

class UnifiedVisionNodeV2:
    def __init__(self):
        rospy.init_node('unified_vision_node_v2', anonymous=True)
        rospy.loginfo("--- Unified Vision Node v2 (Final Hinton Edition) ---")
        ...
        self.path_planning_interval = rospy.Duration(1.0 / 5.0)
        ...
        # 작동 코드의 핵심 원리인 넉넉한 slop 값을 적용합니다.
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer(
            [realsense_img_sub, depth_sub, info_sub], 
            queue_size=5, 
            slop=0.5 # 안정적인 동기화를 위한 핵심 파라미터
        )
        ...
        # [최종 수정] USB 캠 구독자에 tcp_nodelay=True를 적용하여 TCP 반응성을 높입니다.
        usb_cam_topic = '/usb_cam/image_raw/compressed'
        self.usb_cam_sub = rospy.Subscriber(
            usb_cam_topic, 
            CompressedImage, 
            self.usb_cam_callback, 
            queue_size=1, 
            buff_size=2**24,
            tcp_nodelay=True # TCP 지연 감소 옵션 적용
        )
        rospy.loginfo(f"Subscriber for USB Cam started on topic: {usb_cam_topic} with tcp_nodelay")
        rospy.loginfo("✅ Unified Vision Node v2 (Final Corrected) initialized successfully.")
